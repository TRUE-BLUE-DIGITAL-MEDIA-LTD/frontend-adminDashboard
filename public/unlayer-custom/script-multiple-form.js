...
(() => {
    ...
    function recordAnswer(answers, payload) {
        const next = { ...answers };
    ...
    function buildSubmitPayload(answers, email, landingPageId) {
        return { email, landingPageId, formAnswers: answers };
    }
    function buildRedirectUrl(answerUrl, link, fallbackLink, email) {
    ...
    let formLink = "";
    const linkScript = document.getElementsByClassName("script_multiple_form")[0];
    if (linkScript) {
        try {
            const parsed = JSON.parse(linkScript.getAttribute("value") ?? "{}");
     ...
     */
    const wireForm = (scope, rootEl) => {
        const steps = Array.from(scope.getElementsByClassName("form_step"));
        if (steps.length === 0)
            return;
        const landingPageId = rootEl?.getAttribute("data-oxy-lp-id") ?? "";
        const fallbackLink = rootEl?.getAttribute("data-oxy-fallback-link") ?? "";
        const hasSubmitStep = scope.getElementsByClassName("oxy-form-submit-step").length > 0;
        let answers = {};
        ...
        steps.forEach((step, idx) => {
            ...
            for (let c = 0; c < containers.length; c++) {
                const buttons = Array.from(containers[c].children);
                buttons.forEach((button) => {
                    let payload = {};
                    try {
                        payload = JSON.parse(button.getAttribute("value") ?? "{}");
                    }
                    catch {
                    ...
                    button.onclick = () => {
                        ...
                        const toIndex = typeof moveTo === "string" && moveTo !== ""
                            ? Number(moveTo)
                            : i + 1;
        ...
        ctas.forEach((cta) => {
            ...
            if (!stepEl)
                return;
            const emailInput = stepEl.querySelector(".oxy-form-email-input");
            const consent = stepEl.querySelector(".oxy-form-consent-checkbox");
            cta.addEventListener("click", async () => {
                ...
                try {
                    await fetch("/api/v1/customers", {
                        ...
                        body: JSON.stringify(buildSubmitPayload(answers, email, landingPageId)),
                ...
                clearTimeout(timer);
                const redirect = buildRedirectUrl(finalUrl, formLink, fallbackLink, email);
                if (redirect) {
    ...
    const roots = Array.from(document.getElementsByClassName("oxy-multiple-form"));
    if (roots.length > 0) {
