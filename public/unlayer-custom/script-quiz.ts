...
(() => {
  ...
  type RuntimeStep = {
    type: "question" | "interstitial" | "email";
    id: string;
    variant?: "loading" | "message";
    durationMs?: number;
    answers?: RuntimeAnswer[];
  };
  type RuntimeRedirect = { url: string; appendAnswers: boolean };
  type RuntimeConfig = {
    version: number;
    steps: RuntimeStep[];
    redirect: RuntimeRedirect;
    theme?: { showProgressBar?: boolean };
  };
  ...
  function nextStepId(
    ...
    if (goTo && steps.some((s) => s.id === goTo)) return goTo;
  ...
  function progressPercent(
    ...
    const total = steps.filter(
      (s) => s.type === "question" || s.type === "email",
    ).length;
    if (total === 0) return 0;
    let position = 0;
    for (const step of steps) {
      if (step.type === "question" || step.type === "email") position += 1;
      if (step.id === stepId) return Math.round((position / total) * 100);
    }
    return 0;
  }

  (globalThis as Record<string, unknown>).__oxyQuiz = {
    nextStepId,
    buildRedirectUrl,
    isValidEmail,
    progressPercent,
  ...
  function init(): void {
    ...
    const root = document.querySelector<HTMLElement>(".oxy-quiz");
    if (!root) return;
    ...
    if (el.dataset.oxyQuizInit === "1") return;
    el.dataset.oxyQuizInit = "1";

    let config: RuntimeConfig | null = null;
    try {
      const raw = el.getAttribute("data-oxy-quiz-config") || "";
      config = JSON.parse(decodeURIComponent(raw)) as RuntimeConfig;
    } catch (err) {
    ...
    if (!config || !Array.isArray(config.steps) || config.steps.length === 0) {
      ...
      el.querySelectorAll<HTMLButtonElement>(".oxy-quiz__answer").forEach(
        (btn) => {
          btn.onclick = () => {
            if (fallbackUrl) window.open(fallbackUrl, "_self");
          };
        },
      );
      return;
    ...
    function stepEl(id: string): HTMLElement | null {
      return el.querySelector<HTMLElement>(
        `.oxy-quiz__step[data-step-id="${id}"]`,
      );
    }
    ...
    function complete(): void {
      ...
      if (!notClaimed) return; // landing page renderer took over
      if (!redirectUrl) {
    ...
    function showStep(id: string | null): void {
      ...
      cfg.steps.forEach((s) => {
        ...
        if (stepNode) stepNode.style.display = s.id === id ? "flex" : "none";
      });
      const fill = el.querySelector<HTMLElement>(".oxy-quiz__progress-fill");
      if (fill) fill.style.width = `${progressPercent(cfg.steps, id)}%`;

      const step = cfg.steps.find((s) => s.id === id);
      if (step && step.type === "interstitial" && step.variant === "loading") {
        const duration =
          typeof step.durationMs === "number" ? step.durationMs : 2500;
    ...
    function advanceFrom(id: string, goTo: string): void {
    ...
    cfg.steps.forEach((step) => {
      ...
      if (step.type === "question") {
        stepDiv.querySelectorAll<HTMLButtonElement>(".oxy-quiz__answer").forEach(
          (btn) => {
            btn.onclick = () => {
              autoHops = 0;
        ...
        const btn = stepDiv.querySelector<HTMLButtonElement>(".oxy-quiz__continue");
        if (btn) btn.onclick = () => { autoHops = 0; advanceFrom(step.id, ""); };
      } else if (step.type === "email") {
        const input = stepDiv.querySelector<HTMLInputElement>(
          ".oxy-quiz__email-input",
        );
        const error = stepDiv.querySelector<HTMLElement>(".oxy-quiz__email-error");
        const btn = stepDiv.querySelector<HTMLButtonElement>(".oxy-quiz__email-btn");
        if (btn) {
          btn.onclick = () => {
            autoHops = 0;
