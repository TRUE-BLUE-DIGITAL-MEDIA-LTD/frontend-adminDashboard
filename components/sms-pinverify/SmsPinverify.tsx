...
import {
  useCancelSmsPinverify,
  useGetSmsPinverify,
  useGetSmsPinverifyAccounts,
} from "../../react-query";
...
function SmsPinverify({ user }: Props) {
  const activeNumbers = useGetSmsPinverify({ userId: user.id });
  const cancelSMS = useCancelSmsPinverify();
  const accounts = useGetSmsPinverifyAccounts();
    ...
    <>
      <header className="mt-10 flex w-full flex-col items-center justify-center border-b pb-5">
        ...
        <span className="text-sm text-gray-500">
          OxyPin provides the opportunity to use short-term temp phone numbers
