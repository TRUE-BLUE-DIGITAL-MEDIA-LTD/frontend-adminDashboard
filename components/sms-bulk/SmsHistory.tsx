...
import { useGetHistorySmsBulk } from "../../react-query";
import { flagUrl } from "./SelectService";

function SmsHistory() {
  const [page, setPage] = useState(1);
  const history = useGetHistorySmsBulk({ page, limit: 50 });
          ...
          <tbody>
            {history.data?.data.map((sms) => (
              <tr key={sms.id} className="h-16 border-b">
                ...
                <td>
                  <div className="flex items-center justify-center px-2">{sms.phoneNumber}</div>
                </td>
                <td>
                  <div className="flex items-center justify-center gap-1">
                    <div className="relative h-5 w-5 overflow-hidden">
                      <Image src={flagUrl(sms.country)} fill alt="flag" className="object-contain" />
                    </div>
                ...
                <td>
                  <div className="flex items-center justify-center px-2">{sms.serviceCode}</div>
                </td>
              </tr>
            ))}
          </tbody>
