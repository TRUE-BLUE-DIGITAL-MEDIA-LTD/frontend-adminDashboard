import { RiErrorWarningLine } from "react-icons/ri";
import Swal from "sweetalert2";
import {
  ErrorMessages,
  Partner,
  User,
  SmsGetatextAccount,
  SmsGetatext,
  SmsGetatextMessage,
} from "../../models";
import {
  useCancelSmsGetatext,
  useGetActiveSmsGetatextNumbers,
  useGetSmsGetatextAccounts,
  useGetSmsGetatextBalance,
} from "../../react-query/sms-getatext";
import SelectService from "./SelectService";
import SmsGetatextAccountCard from "./SmsGetatextAccount";
import SmsGetatextCard from "./SmsGetatextCard";
import SmsGetatextDelayReport from "./SmsGetatextDelayReport";
import SmsGetatextHistory from "./SmsGetatextHistory";

type Props = {
  user: User & { partner: Partner | null };
};

function SmsGetatexts({ user }: Props) {
  const activeNumbers = useGetActiveSmsGetatextNumbers();
  const cancelSMS = useCancelSmsGetatext();
  const accounts = useGetSmsGetatextAccounts();
  const balance = useGetSmsGetatextBalance();

  const handleCancelSMS = async (id: string) => {
    try {
      Swal.fire({
        title: "Loading",
        html: "Please wait.",
        allowEscapeKey: false,
        allowOutsideClick: false,
        didOpen: () => {
          Swal.showLoading();
        },
      });
      await cancelSMS.mutateAsync({
        id: id,
      });
      await activeNumbers.refetch();
      Swal.fire({
        title: "Success",
        text: "Operation has been successfully completed.",
        icon: "success",
      });
    } catch (error) {
      console.log(error);
      let result = error as ErrorMessages;
      Swal.fire({
        title: result.error ? result.error : "Something went wrong!",
        text: result.message?.toString(),
        footer: result.statusCode
          ? "Error code: " + result.statusCode?.toString()
          : "",
        icon: "error",
      });
    }
  };

  return (
    <>
      <header className="mt-10 flex w-full flex-col items-center justify-center border-b pb-5">
        {(user.role === "manager" ||
          user.role === "admin" ||
          user.partner?.isAllowManageSmsGetatextAccount === true) && (
          <ul className="flex w-full flex-wrap items-center justify-center gap-3">
            {accounts.data?.map((a: SmsGetatextAccount) => {
              return (
                <SmsGetatextAccountCard key={a.id} account={a} user={user} />
              );
            })}
          </ul>
        )}
        {(user.role === "manager" || user.role === "admin") && (
          <div className="mt-2 flex w-full flex-col items-center justify-center gap-2">
            <span className="rounded-md bg-gray-100 px-3 py-1 font-semibold text-gray-800 ring-1 ring-gray-300">
              Balance: {balance.data?.balance}
            </span>
            <SmsGetatextDelayReport />
          </div>
        )}
        <h1 className="text-4xl font-semibold text-gray-800">OxyGT</h1>
        <span className="text-sm text-gray-500">
          OxyGT provides real US Non-VoIP phone numbers for SMS verification.
        </span>
      </header>
      <main className="mt-5 flex w-full flex-col items-center gap-5 pb-20">
        <section className="flex w-10/12  flex-col items-start  justify-start gap-5 ">
          <h1 className="text-lg font-semibold">My numbers</h1>
          {!activeNumbers.data ||
            (activeNumbers.data.length === 0 && (
              <div className="flex  w-96 flex-col items-center justify-center gap-5 rounded-md bg-white p-5 ring-1 ring-gray-400 drop-shadow-xl">
                <RiErrorWarningLine className="text-5xl" />
                <h3 className="text-xl">No operations.</h3>
                <span className="text-sm">
                  Order a number and use it to register in the selected
                  app/website
                </span>
              </div>
            ))}
          <ul className=" grid w-full grid-cols-1 gap-5 md:grid-cols-2 2xl:grid-cols-3">
            {activeNumbers.data?.map(
              (number: SmsGetatext & { messages: SmsGetatextMessage[] }) => {
                return (
                  <SmsGetatextCard
                    key={number.id}
                    smsGetatext={number}
                    onCancel={handleCancelSMS}
                  />
                );
              },
            )}
          </ul>
        </section>
        <section className="flex w-full flex-col items-center justify-center gap-5">
          <SelectService activeNumbers={activeNumbers} />
          <SmsGetatextHistory activeNumbers={activeNumbers} />
        </section>
      </main>
    </>
  );
}

export default SmsGetatexts;
